    ...
    PROCEDURE GET_DEPT_PATIENTS (p_hospital_id IN NUMBER, p_department_id IN NUMBER) AS
        ...
        CURSOR c_patients IS
            SELECT  p.PATIENT_ID, p.PATIENT_FIRST_NAME, p.PATIENT_LAST_NAME,
              ...
              LEFT JOIN HMS_PATIENT_PHONE_MST_CH pp ON p.PATIENT_ID = pp.PATIENT_ID AND pp.PHONE_TYPE = 'PRIMARY'
             WHERE  p.HOSPITAL_ID = p_hospital_id AND p.DEPARTMENT_ID = p_department_id
             ORDER BY p.PATIENT_ID;
    BEGIN
        BEGIN SELECT BRANCH_NAME INTO v_branch_name FROM HMS_HOSPITAL_BRANCH_CH WHERE HOSPITAL_ID = p_hospital_id;
        EXCEPTION WHEN NO_DATA_FOUND THEN DBMS_OUTPUT.PUT_LINE('ERROR: Hospital ID not found.'); RETURN; END;
        BEGIN SELECT DEPARTMENT_NAME INTO v_dept_name FROM HMS_DEPARTMENT_CH WHERE DEPARTMENT_ID = p_department_id AND HOSPITAL_ID = p_hospital_id;
    ...
    BEGIN
        ...
        FOR r IN c_pat LOOP
            ...
            IF v_error_msg IS NOT NULL THEN
                ...
                v_error_count := v_error_count + 1;
            ELSE
                INSERT INTO HMS_PATIENT_CH (PATIENT_ID,HOSPITAL_ID,DEPARTMENT_ID,PATIENT_FIRST_NAME,PATIENT_LAST_NAME,EMAIL_ID,ADDRESS_STREET,ADDRESS_CITY,ADDRESS_STATE,ADDRESS_POSTAL_CODE) VALUES (r.PATIENT_ID,r.HOSPITAL_ID,r.DEPARTMENT_ID,r.PATIENT_FIRST_NAME,r.PATIENT_LAST_NAME,r.EMAIL_ID,r.ADDRESS_STREET,r.ADDRESS_CITY,r.ADDRESS_STATE,r.ADDRESS_POSTAL_CODE);
