...
CREATE OR REPLACE PACKAGE BODY HMS_PKG_MD AS
    ...
    BEGIN
        BEGIN SELECT BRANCH_NAME INTO v_branch_name FROM HMS_HOSPITAL_BRANCH_MD WHERE HOSPITAL_ID = p_hospital_id;
    ...
    PROCEDURE GET_DEPT_PATIENTS (p_hospital_id IN NUMBER, p_department_id IN NUMBER) AS
        ...
        CURSOR c_patients IS
            SELECT  p.PATIENT_ID, p.PATIENT_FIRST_NAME, p.PATIENT_LAST_NAME,
              ...
              LEFT JOIN HMS_PATIENT_PHONE_MST_MD pp ON p.PATIENT_ID = pp.PATIENT_ID AND pp.PHONE_TYPE = 'PRIMARY'
    ...
    BEGIN
        ...
        BEGIN SELECT DEPARTMENT_NAME INTO v_dept_name FROM HMS_DEPARTMENT_MD WHERE DEPARTMENT_ID = p_department_id AND HOSPITAL_ID = p_hospital_id;
    ...
    BEGIN
        DBMS_OUTPUT.PUT_LINE('==============================================');
        DBMS_OUTPUT.PUT_LINE('STAGING-TO-BASE LOAD  |  Member: MD (Manideep)');
        ...
        FOR r IN c_hosp_master LOOP
            ...
            IF v_error_msg IS NULL THEN SELECT COUNT(*) INTO v_dup_count FROM HMS_HOSPITAL_MASTER_MD WHERE HOSPITAL_CODE = r.HOSPITAL_CODE; IF v_dup_count > 0 THEN v_error_msg := 'Duplicate HOSPITAL_CODE. '; END IF; END IF;
            IF v_error_msg IS NULL AND NVL(r.HOSPITAL_BASIC_FEES,0) <= 0 THEN v_error_msg := 'HOSPITAL_BASIC_FEES must be > 0. '; END IF;
            IF v_error_msg IS NOT NULL THEN UPDATE HMS_HOSPITAL_MASTER_STG_MD SET RECORD_STATUS='ERROR',ERROR_LOG=v_error_msg,LAST_UPDATED_BY=C_USER_ID,LAST_UPDATE_DATE=v_now,LAST_UPDATE_LOGIN=v_login_id WHERE STG_ID=r.STG_ID; v_error_count:=v_error_count+1;
            ELSE INSERT INTO HMS_HOSPITAL_MASTER_MD (HOSPITAL_CODE,CITY_NAME,HOSPITAL_NAME,HOSPITAL_BASIC_FEES) VALUES (r.HOSPITAL_CODE,r.CITY_NAME,r.HOSPITAL_NAME,r.HOSPITAL_BASIC_FEES); UPDATE HMS_HOSPITAL_MASTER_STG_MD SET RECORD_STATUS='LOADED',ERROR_LOG=NULL,LAST_UPDATED_BY=C_USER_ID,LAST_UPDATE_DATE=v_now,LAST_UPDATE_LOGIN=v_login_id WHERE STG_ID=r.STG_ID; v_loaded_count:=v_loaded_count+1; END IF;
        END LOOP;
        FOR r IN c_dept LOOP
            ...
            IF v_error_msg IS NOT NULL THEN UPDATE HMS_DEPARTMENT_STG_MD SET RECORD_STATUS='ERROR',ERROR_LOG=v_error_msg,LAST_UPDATED_BY=C_USER_ID,LAST_UPDATE_DATE=v_now,LAST_UPDATE_LOGIN=v_login_id WHERE STG_ID=r.STG_ID; v_error_count:=v_error_count+1;
            ELSE INSERT INTO HMS_DEPARTMENT_MD (DEPARTMENT_ID,HOSPITAL_ID,DEPARTMENT_NAME,DEPT_MANAGER,NUMBER_OF_BEDS) VALUES (r.DEPARTMENT_ID,r.HOSPITAL_ID,r.DEPARTMENT_NAME,r.DEPT_MANAGER,NVL(r.NUMBER_OF_BEDS,0)); UPDATE HMS_DEPARTMENT_STG_MD SET RECORD_STATUS='LOADED',ERROR_LOG=NULL,LAST_UPDATED_BY=C_USER_ID,LAST_UPDATE_DATE=v_now,LAST_UPDATE_LOGIN=v_login_id WHERE STG_ID=r.STG_ID; v_loaded_count:=v_loaded_count+1; END IF;
        END LOOP;
        FOR r IN c_emp LOOP
            v_error_msg := NULL;
            ...
            IF v_error_msg IS NOT NULL THEN UPDATE HMS_EMPLOYEES_STG_MD SET RECORD_STATUS='ERROR',ERROR_LOG=v_error_msg,LAST_UPDATED_BY=C_USER_ID,LAST_UPDATE_DATE=v_now,LAST_UPDATE_LOGIN=v_login_id WHERE STG_ID=r.STG_ID; v_error_count:=v_error_count+1;
            ELSE INSERT INTO HMS_EMPLOYEES_MD (EMPLOYEE_ID,HOSPITAL_ID,DEPARTMENT_ID,EMPLOYEE_FIRST_NAME,EMPLOYEE_LAST_NAME,EMPLOYEE_TYPE,EMAIL_ID) VALUES (r.EMPLOYEE_ID,r.HOSPITAL_ID,r.DEPARTMENT_ID,r.EMPLOYEE_FIRST_NAME,r.EMPLOYEE_LAST_NAME,r.EMPLOYEE_TYPE,r.EMAIL_ID); UPDATE HMS_EMPLOYEES_STG_MD SET RECORD_STATUS='LOADED',ERROR_LOG=NULL,LAST_UPDATED_BY=C_USER_ID,LAST_UPDATE_DATE=v_now,LAST_UPDATE_LOGIN=v_login_id WHERE STG_ID=r.STG_ID; v_loaded_count:=v_loaded_count+1; END IF;
        END LOOP;
        FOR r IN c_pat LOOP
            v_error_msg := NULL;
            IF r.PATIENT_FIRST_NAME IS NULL OR r.PATIENT_LAST_NAME IS NULL THEN v_error_msg := 'Patient name cannot be NULL. '; END IF;
            IF v_error_msg IS NOT NULL THEN UPDATE HMS_PATIENT_STG_MD SET RECORD_STATUS='ERROR',ERROR_LOG=v_error_msg,LAST_UPDATED_BY=C_USER_ID,LAST_UPDATE_DATE=v_now,LAST_UPDATE_LOGIN=v_login_id WHERE STG_ID=r.STG_ID; v_error_count:=v_error_count+1;
            ELSE INSERT INTO HMS_PATIENT_MD (PATIENT_ID,HOSPITAL_ID,DEPARTMENT_ID,PATIENT_FIRST_NAME,PATIENT_LAST_NAME,EMAIL_ID,ADDRESS_STREET,ADDRESS_CITY,ADDRESS_STATE,ADDRESS_POSTAL_CODE) VALUES (r.PATIENT_ID,r.HOSPITAL_ID,r.DEPARTMENT_ID,r.PATIENT_FIRST_NAME,r.PATIENT_LAST_NAME,r.EMAIL_ID,r.ADDRESS_STREET,r.ADDRESS_CITY,r.ADDRESS_STATE,r.ADDRESS_POSTAL_CODE); UPDATE HMS_PATIENT_STG_MD SET RECORD_STATUS='LOADED',ERROR_LOG=NULL,LAST_UPDATED_BY=C_USER_ID,LAST_UPDATE_DATE=v_now,LAST_UPDATE_LOGIN=v_login_id WHERE STG_ID=r.STG_ID; v_loaded_count:=v_loaded_count+1; END IF;
        END LOOP;
        COMMIT;
