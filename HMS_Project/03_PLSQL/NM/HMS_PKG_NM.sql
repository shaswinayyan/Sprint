...
CREATE OR REPLACE PACKAGE HMS_PKG_NM AS
    C_USER_ID   CONSTANT NUMBER := 1021052;
...
CREATE OR REPLACE PACKAGE BODY HMS_PKG_NM AS
    ...
    BEGIN
        BEGIN
            SELECT BRANCH_NAME INTO v_branch_name
              FROM HMS_HOSPITAL_BRANCH_NM WHERE HOSPITAL_ID = p_hospital_id;
        EXCEPTION
    ...
    BEGIN
        BEGIN
            SELECT BRANCH_NAME INTO v_branch_name FROM HMS_HOSPITAL_BRANCH_NM WHERE HOSPITAL_ID = p_hospital_id;
        EXCEPTION WHEN NO_DATA_FOUND THEN
            FND_FILE.PUT_LINE(FND_FILE.OUTPUT, 'ERROR: Hospital ID ' || p_hospital_id || ' not found.'); RETURN;
        END;
        BEGIN
            SELECT DEPARTMENT_NAME INTO v_dept_name FROM HMS_DEPARTMENT_NM
    ...
    BEGIN
        ...
        -- [1/4] Hospital Master
        FND_FILE.PUT_LINE(FND_FILE.OUTPUT, '[1/4] Processing HMS_HOSPITAL_MASTER_STG_NM ...');
        FOR r IN c_hosp_master LOOP
            ...
            IF v_error_msg IS NULL THEN
                SELECT COUNT(*) INTO v_dup_count FROM HMS_HOSPITAL_MASTER_NM WHERE HOSPITAL_CODE = r.HOSPITAL_CODE;
            ...
            IF v_error_msg IS NOT NULL THEN
                UPDATE HMS_HOSPITAL_MASTER_STG_NM SET RECORD_STATUS='ERROR', ERROR_LOG=v_error_msg, LAST_UPDATED_BY=C_USER_ID, LAST_UPDATE_DATE=v_now, LAST_UPDATE_LOGIN=v_login_id WHERE STG_ID=r.STG_ID;
                v_error_count := v_error_count + 1;
            ELSE
                INSERT INTO HMS_HOSPITAL_MASTER_NM (HOSPITAL_CODE, CITY_NAME, HOSPITAL_NAME, HOSPITAL_BASIC_FEES) VALUES (r.HOSPITAL_CODE, r.CITY_NAME, r.HOSPITAL_NAME, r.HOSPITAL_BASIC_FEES);
                UPDATE HMS_HOSPITAL_MASTER_STG_NM SET RECORD_STATUS='LOADED', ERROR_LOG=NULL, LAST_UPDATED_BY=C_USER_ID, LAST_UPDATE_DATE=v_now, LAST_UPDATE_LOGIN=v_login_id WHERE STG_ID=r.STG_ID;
                v_loaded_count := v_loaded_count + 1;
            END IF;
        END LOOP;

        -- [1.5/4] Hospital Branch
        FND_FILE.PUT_LINE(FND_FILE.OUTPUT, '[1.5/4] Processing HMS_HOSPITAL_BRANCH_STG_NM ...');
        FOR r IN (SELECT * FROM HMS_HOSPITAL_BRANCH_STG_NM WHERE RECORD_STATUS='NEW' AND (p_batch_id IS NULL OR BATCH_ID=p_batch_id) ORDER BY STG_ID) LOOP
            v_error_msg := NULL;
            IF r.HOSPITAL_CODE IS NULL OR r.BRANCH_NAME IS NULL THEN v_error_msg := 'HOSPITAL_CODE or BRANCH_NAME is NULL. '; END IF;
            IF v_error_msg IS NOT NULL THEN
                UPDATE HMS_HOSPITAL_BRANCH_STG_NM SET RECORD_STATUS='ERROR', ERROR_LOG=v_error_msg, LAST_UPDATED_BY=C_USER_ID, LAST_UPDATE_DATE=v_now, LAST_UPDATE_LOGIN=v_login_id WHERE STG_ID=r.STG_ID;
                v_error_count := v_error_count + 1;
            ELSE
                INSERT INTO HMS_HOSPITAL_BRANCH_NM (HOSPITAL_ID, HOSPITAL_CODE, BRANCH_NAME, CITY, MANAGING_DIRECTOR, HELPDESK_NUMBER, EMERGENCY_NUMBER, CUSTOMER_CARE_EMAIL, CUSTOMER_CARE_PHONE)
                ...
                UPDATE HMS_HOSPITAL_BRANCH_STG_NM SET RECORD_STATUS='LOADED', ERROR_LOG=NULL, LAST_UPDATED_BY=C_USER_ID, LAST_UPDATE_DATE=v_now, LAST_UPDATE_LOGIN=v_login_id WHERE STG_ID=r.STG_ID;
                v_loaded_count := v_loaded_count + 1;
            END IF;
        END LOOP;

        -- [2/4] Departments
        FND_FILE.PUT_LINE(FND_FILE.OUTPUT, '[2/4] Processing HMS_DEPARTMENT_STG_NM ...');
        FOR r IN c_dept LOOP
            ...
            IF v_error_msg IS NOT NULL THEN
                UPDATE HMS_DEPARTMENT_STG_NM SET RECORD_STATUS='ERROR', ERROR_LOG=v_error_msg, LAST_UPDATED_BY=C_USER_ID, LAST_UPDATE_DATE=v_now, LAST_UPDATE_LOGIN=v_login_id WHERE STG_ID=r.STG_ID;
                v_error_count := v_error_count + 1;
            ELSE
                INSERT INTO HMS_DEPARTMENT_NM (DEPARTMENT_ID, HOSPITAL_ID, DEPARTMENT_NAME, DEPT_MANAGER, NUMBER_OF_BEDS) VALUES (r.DEPARTMENT_ID, r.HOSPITAL_ID, r.DEPARTMENT_NAME, r.DEPT_MANAGER, NVL(r.NUMBER_OF_BEDS, 0));
                UPDATE HMS_DEPARTMENT_STG_NM SET RECORD_STATUS='LOADED', ERROR_LOG=NULL, LAST_UPDATED_BY=C_USER_ID, LAST_UPDATE_DATE=v_now, LAST_UPDATE_LOGIN=v_login_id WHERE STG_ID=r.STG_ID;
                v_loaded_count := v_loaded_count + 1;
            END IF;
        END LOOP;

        -- [3/4] Employees
        FND_FILE.PUT_LINE(FND_FILE.OUTPUT, '[3/4] Processing HMS_EMPLOYEES_STG_NM ...');
        FOR r IN c_emp LOOP
            v_error_msg := NULL;
            ...
            IF v_error_msg IS NOT NULL THEN
                UPDATE HMS_EMPLOYEES_STG_NM SET RECORD_STATUS='ERROR', ERROR_LOG=v_error_msg, LAST_UPDATED_BY=C_USER_ID, LAST_UPDATE_DATE=v_now, LAST_UPDATE_LOGIN=v_login_id WHERE STG_ID=r.STG_ID;
                v_error_count := v_error_count + 1;
            ELSE
                INSERT INTO HMS_EMPLOYEES_NM (EMPLOYEE_ID, HOSPITAL_ID, DEPARTMENT_ID, EMPLOYEE_FIRST_NAME, EMPLOYEE_LAST_NAME, EMPLOYEE_TYPE, EMAIL_ID) VALUES (r.EMPLOYEE_ID, r.HOSPITAL_ID, r.DEPARTMENT_ID, r.EMPLOYEE_FIRST_NAME, r.EMPLOYEE_LAST_NAME, r.EMPLOYEE_TYPE, r.EMAIL_ID);
                UPDATE HMS_EMPLOYEES_STG_NM SET RECORD_STATUS='LOADED', ERROR_LOG=NULL, LAST_UPDATED_BY=C_USER_ID, LAST_UPDATE_DATE=v_now, LAST_UPDATE_LOGIN=v_login_id WHERE STG_ID=r.STG_ID;
                v_loaded_count := v_loaded_count + 1;
            END IF;
        END LOOP;

        -- [3.5/7] Employee Phone Master
        FND_FILE.PUT_LINE(FND_FILE.OUTPUT, '[3.5/7] Processing HMS_EMPLOYEE_PHONE_MST_STG_NM ...');
        FOR r IN (SELECT * FROM HMS_EMPLOYEE_PHONE_MST_STG_NM WHERE RECORD_STATUS='NEW' AND (p_batch_id IS NULL OR BATCH_ID=p_batch_id) ORDER BY STG_ID) LOOP
            v_error_msg := NULL;
            ...
            IF v_error_msg IS NOT NULL THEN
                UPDATE HMS_EMPLOYEE_PHONE_MST_STG_NM SET RECORD_STATUS='ERROR', ERROR_LOG=v_error_msg, LAST_UPDATED_BY=C_USER_ID, LAST_UPDATE_DATE=v_now, LAST_UPDATE_LOGIN=v_login_id WHERE STG_ID=r.STG_ID;
                v_error_count := v_error_count + 1;
            ELSE
                INSERT INTO HMS_EMPLOYEE_PHONE_MST_NM (PHONE_RECORD_ID, EMPLOYEE_ID, PHONE1, PHONE2) VALUES (r.PHONE_RECORD_ID, r.EMPLOYEE_ID, r.PHONE1, r.PHONE2);
                UPDATE HMS_EMPLOYEE_PHONE_MST_STG_NM SET RECORD_STATUS='LOADED', ERROR_LOG=NULL, LAST_UPDATED_BY=C_USER_ID, LAST_UPDATE_DATE=v_now, LAST_UPDATE_LOGIN=v_login_id WHERE STG_ID=r.STG_ID;
                v_loaded_count := v_loaded_count + 1;
            END IF;
        END LOOP;

        -- [3.6/7] Doctor Availability
        FND_FILE.PUT_LINE(FND_FILE.OUTPUT, '[3.6/7] Processing HMS_DOCTOR_AVAILABILITY_STG_NM ...');
        FOR r IN (SELECT * FROM HMS_DOCTOR_AVAILABILITY_STG_NM WHERE RECORD_STATUS='NEW' AND (p_batch_id IS NULL OR BATCH_ID=p_batch_id) ORDER BY STG_ID) LOOP
            v_error_msg := NULL;
            ...
            IF v_error_msg IS NOT NULL THEN
                UPDATE HMS_DOCTOR_AVAILABILITY_STG_NM SET RECORD_STATUS='ERROR', ERROR_LOG=v_error_msg, LAST_UPDATED_BY=C_USER_ID, LAST_UPDATE_DATE=v_now, LAST_UPDATE_LOGIN=v_login_id WHERE STG_ID=r.STG_ID;
                v_error_count := v_error_count + 1;
            ELSE
                INSERT INTO HMS_DOCTOR_AVAILABILITY_NM (AVAILABILITY_ID, DOCTOR_ID, DOCTOR_DEPARTMENT, AVAILABILITY_DAY, START_TIME, END_TIME) VALUES (r.AVAILABILITY_ID, r.DOCTOR_ID, r.DOCTOR_DEPARTMENT, r.AVAILABILITY_DAY, r.START_TIME, r.END_TIME);
                UPDATE HMS_DOCTOR_AVAILABILITY_STG_NM SET RECORD_STATUS='LOADED', ERROR_LOG=NULL, LAST_UPDATED_BY=C_USER_ID, LAST_UPDATE_DATE=v_now, LAST_UPDATE_LOGIN=v_login_id WHERE STG_ID=r.STG_ID;
                v_loaded_count := v_loaded_count + 1;
            END IF;
        END LOOP;

        -- [4/4] Patients
        FND_FILE.PUT_LINE(FND_FILE.OUTPUT, '[4/4] Processing HMS_PATIENT_STG_NM ...');
        FOR r IN c_pat LOOP
            v_error_msg := NULL;
            ...
            IF v_error_msg IS NOT NULL THEN
                UPDATE HMS_PATIENT_STG_NM SET RECORD_STATUS='ERROR', ERROR_LOG=v_error_msg, LAST_UPDATED_BY=C_USER_ID, LAST_UPDATE_DATE=v_now, LAST_UPDATE_LOGIN=v_login_id WHERE STG_ID=r.STG_ID;
                v_error_count := v_error_count + 1;
            ELSE
                INSERT INTO HMS_PATIENT_NM (PATIENT_ID, HOSPITAL_ID, DEPARTMENT_ID, PATIENT_FIRST_NAME, PATIENT_LAST_NAME, PATIENT_PHONE_NUMBER, EMAIL_ID, ADDRESS_STREET, ADDRESS_CITY, ADDRESS_STATE, ADDRESS_POSTAL_CODE)
                ...
                UPDATE HMS_PATIENT_STG_NM SET RECORD_STATUS='LOADED', ERROR_LOG=NULL, LAST_UPDATED_BY=C_USER_ID, LAST_UPDATE_DATE=v_now, LAST_UPDATE_LOGIN=v_login_id WHERE STG_ID=r.STG_ID;
                v_loaded_count := v_loaded_count + 1;
            END IF;
        END LOOP;
...
-- EXEC HMS_PKG_NM.GET_EMPLOYEES_LIST(1);
-- EXEC HMS_PKG_NM.GET_DEPT_PATIENTS(1, 1);
-- EXEC HMS_PKG_NM.LOAD_STAGING_TO_BASE();
-- EXEC HMS_PKG_NM.LOAD_STAGING_TO_BASE('BATCH_NM_20260321');
-- SELECT STG_ID, RECORD_STATUS, ERROR_LOG, LAST_UPDATED_BY, LAST_UPDATE_DATE
--   FROM HMS_HOSPITAL_MASTER_STG_NM ORDER BY STG_ID;
-- ============================================================
