...
CREATE OR REPLACE PACKAGE BODY HMS_PKG_NM AS
    ...
    PROCEDURE GET_DEPT_PATIENTS (p_hospital_id IN NUMBER, p_department_id IN NUMBER) AS
        ...
        CURSOR c_patients IS
            SELECT  p.PATIENT_ID, p.PATIENT_FIRST_NAME, p.PATIENT_LAST_NAME,
              ...
              LEFT JOIN HMS_PATIENT_PHONE_MST_NM pp ON p.PATIENT_ID = pp.PATIENT_ID AND pp.PHONE_TYPE = 'PRIMARY'
    ...
    BEGIN
        ...
        FOR r IN c_pat LOOP
            ...
            ELSE INSERT INTO HMS_PATIENT_NM (PATIENT_ID,HOSPITAL_ID,DEPARTMENT_ID,PATIENT_FIRST_NAME,PATIENT_LAST_NAME,EMAIL_ID,ADDRESS_STREET,ADDRESS_CITY,ADDRESS_STATE,ADDRESS_POSTAL_CODE) VALUES (r.PATIENT_ID,r.HOSPITAL_ID,r.DEPARTMENT_ID,r.PATIENT_FIRST_NAME,r.PATIENT_LAST_NAME,r.EMAIL_ID,r.ADDRESS_STREET,r.ADDRESS_CITY,r.ADDRESS_STATE,r.ADDRESS_POSTAL_CODE); UPDATE HMS_PATIENT_STG_NM SET RECORD_STATUS='LOADED',ERROR_LOG=NULL,LAST_UPDATED_BY=C_USER_ID,LAST_UPDATE_DATE=v_now,LAST_UPDATE_LOGIN=v_login_id WHERE STG_ID=r.STG_ID; v_loaded_count:=v_loaded_count+1; END IF;
        END LOOP;
        COMMIT;
